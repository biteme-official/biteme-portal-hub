...
export default function ApprovalDetailPage() {
  ...
  const [yesNoVote, setYesNoVote] = useState<boolean | null>(null);
  ...
  async function handleApprove() {
    if (!approval) return;
    if (approval.voteType === "options" && !selectedOption) {
      alert("안건을 선택해주세요.");
      return;
    }
    if (approval.voteType === "yesno" && yesNoVote === null) {
      alert("찬성/반대를 선택해주세요.");
      return;
    }
    ...
    await fetch(`/api/approvals/${id}/decide`, {
      ...
      body: JSON.stringify({
        action: "approve",
        ...(approval.voteType === "options" ? { selectedOption } : {}),
        ...(approval.voteType === "yesno" ? { yesNoVote } : {}),
      }),
    });
    fetchApproval();
  ...
  async function handleReject() {
    ...
    await fetch(`/api/approvals/${id}/decide`, {
      ...
      body: JSON.stringify({
        ...
        ...(approval.voteType === "options" && selectedOption ? { selectedOption } : {}),
        ...(approval.voteType === "yesno" && yesNoVote !== null ? { yesNoVote } : {}),
      }),
    });
    fetchApproval();
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      ...
      <div className="space-y-4">
        ...
        {/* Vote Summary */}
        {approval.voteType !== "none" && approval.approvalLine.some(s => s.status === "approved" || s.status === "rejected") && (
          <div className="bg-white rounded-xl border border-border p-5">
            <h2 className="text-sm font-semibold text-text-primary mb-3">
              {approval.voteType === "options" ? "📊 안건 선택 현황" : "📊 Y/N 투표 현황"}
            </h2>
            {approval.voteType === "options" && (
              <div className="space-y-2">
                {approval.voteOptions.map((opt) => {
                  const voters = approval.approvalLine.filter(s => s.selectedOption === opt.id && (s.status === "approved" || s.status === "rejected"));
                  const total = approval.approvalLine.filter(s => s.status === "approved" || s.status === "rejected").length;
                    ...
                    <div key={opt.id} className="flex items-center gap-3">
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-xs text-text-secondary">{voters.length}표 ({pct}%)</span>
                        </div>
                        ...
                        {voters.length > 0 && (
                          <div className="flex gap-1 mt-1">
                            {voters.map(v => (
                              <span key={v.approver.uid} className="text-[10px] text-text-secondary bg-surface px-1.5 py-0.5 rounded">
                                {v.approver.name}
                              </span>
                            ))}
                          </div>
            ...
            {approval.voteType === "yesno" && (() => {
              const decided = approval.approvalLine.filter(s => s.status === "approved" || s.status === "rejected");
              const yesVotes = decided.filter(s => s.yesNoVote === true);
              const noVotes = decided.filter(s => s.yesNoVote === false);
              const total = decided.length;
              return (
                <div className="flex gap-4">
                  <div className="flex-1 text-center p-3 rounded-lg bg-green-50 border border-green-200">
                    <div className="text-lg font-bold text-green-600">{yesVotes.length}</div>
                    <div className="text-xs text-green-600 font-medium">찬성 (Y)</div>
                    {yesVotes.length > 0 && (
                      <div className="flex flex-wrap justify-center gap-1 mt-1.5">
                        {yesVotes.map(v => (
                          <span key={v.approver.uid} className="text-[10px] text-green-700 bg-green-100 px-1.5 py-0.5 rounded">{v.approver.name}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  <div className="flex-1 text-center p-3 rounded-lg bg-red-50 border border-red-200">
                    <div className="text-lg font-bold text-red-600">{noVotes.length}</div>
                    <div className="text-xs text-red-600 font-medium">반대 (N)</div>
                    {noVotes.length > 0 && (
                      <div className="flex flex-wrap justify-center gap-1 mt-1.5">
                        {noVotes.map(v => (
                          <span key={v.approver.uid} className="text-[10px] text-red-700 bg-red-100 px-1.5 py-0.5 rounded">{v.approver.name}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  {total > 0 && (
                    <div className="flex-1 text-center p-3 rounded-lg bg-gray-50 border border-border">
                      <div className="text-lg font-bold text-text-primary">{total}</div>
                      <div className="text-xs text-text-secondary font-medium">총 투표</div>
                    </div>
                  )}
                </div>
              );
        ...
        {(canApprove || canCancel || approval.status === "draft" || isAdmin) && (
          <div className="bg-white rounded-xl border border-border p-5">
            {/* Vote Selection UI for current approver */}
            {canApprove && approval.voteType === "options" && (
              <div className="mb-4 pb-4 border-b border-border">
                <p className="text-sm font-medium text-text-primary mb-2">안건을 선택해주세요 *</p>
                <div className="space-y-2">
                  {approval.voteOptions.map((opt) => (
                    <button
                      key={opt.id}
            ...
            {canApprove && approval.voteType === "yesno" && (
              <div className="mb-4 pb-4 border-b border-border">
                <p className="text-sm font-medium text-text-primary mb-2">의견을 선택해주세요 *</p>
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => setYesNoVote(true)}
                    className={`flex-1 py-2.5 rounded-lg text-sm font-medium border-2 transition-colors ${
                      yesNoVote === true
                        ? "border-green-500 bg-green-50 text-green-700"
                        : "border-border text-text-secondary hover:border-green-300"
                    }`}
                  >
                    👍 찬성 (Y)
                  </button>
